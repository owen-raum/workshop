export function ForWhom() {
  ...
  return (
    <section id="whom" className="py-20 md:py-32 lg:py-40 px-4 bg-navy-50">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-12">
          <h3 className="text-xl md:text-2xl font-bold text-navy-900 mb-6 md:mb-8 text-center px-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            {perfectFor.map((item, index) => (
              <div
                key={index}
                className="bg-white rounded-2xl p-6 border border-navy-200 hover:border-orange-300 hover:shadow-lg transition-all"
              >
                <div className="w-12 h-12 md:w-14 md:h-14 bg-orange-100 rounded-xl flex items-center justify-center text-orange-600 mb-4">
                  {item.icon}
                </div>
                <h4 className="text-base md:text-lg font-bold text-navy-900 mb-2 md:mb-3">
                  {item.title}
                </h4>
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl p-6 md:p-8 lg:p-10 border border-navy-200">
            <ul className="space-y-4">
              {notFor.map((item, index) => (
                <li key={index} className="flex items-start gap-3 md:gap-4">
                  <div className="flex-shrink-0 w-6 h-6 rounded-full bg-navy-200 flex items-center justify-center">
                    <svg className="w-4 h-4 text-navy-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                    </svg>
                  </div>
                  <span className="text-navy-800 text-base md:text-lg">
                    {item}
                  </span>
                </li>
              ))}
        ...
        <div className="mt-12 text-center bg-white rounded-2xl p-6 md:p-8 border border-navy-200">
          <p className="text-base md:text-lg text-navy-800 font-medium px-2">
